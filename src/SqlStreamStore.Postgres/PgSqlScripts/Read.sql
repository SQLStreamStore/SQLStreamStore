...
BEGIN
  ...
  SELECT
    __schema__.streams.version  as stream_version,
  ...
  OPEN _messages FOR
  SELECT
    __schema__.streams.id_original AS stream_id,
  ...
  WHERE
    (
      CASE WHEN _forwards
        THEN
          __schema__.messages.stream_version >= _version AND id_internal = _stream_id_internal
      ELSE
        __schema__.messages.stream_version <= _version AND id_internal = _stream_id_internal
      END
    )
