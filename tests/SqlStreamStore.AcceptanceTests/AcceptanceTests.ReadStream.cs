namespace SqlStreamStore
{
    ...
    public partial class AcceptanceTests
    {
        ...
        public async Task Can_read_streams_forwards_with_prefetch(ReadStreamTheory theory)
        {
            ...
            var streamMessagesPage = store.ReadStreamForwards(theory.StreamId, theory.Start, theory.PageSize);
        ...
        public async Task Can_read_next_page_past_end_of_stream()
        {
            ...
            var page = store.ReadStreamForwards("stream-1", StreamVersion.Start, 4);
        ...
        */
        ...
        public async Task Can_read_all_messages()
        {
            ...
            var page = store.ReadStreamForwards("stream-1", StreamVersion.Start, int.MaxValue);
        ...
        public async Task Can_read_streams_backwards_with_prefetch(ReadStreamTheory theory)
        {
            ...
            var streamMessagesPage = store.ReadStreamBackwards(theory.StreamId, theory.Start, theory.PageSize);
        ...
        public async Task Can_read_empty_stream_backwards(bool prefetch)
        {
            ...
            var page = store.ReadStreamBackwards("stream-1", StreamVersion.End, 1, prefetch);
            var messages = await page.ToArrayAsync();
        ...
        public async Task Can_read_empty_stream_forwards(bool prefetch)
        {
            ...
            var page = store.ReadStreamForwards("stream-1", StreamVersion.Start, 1, prefetch);
            var messages = await page.ToArrayAsync();
            page.Status.ShouldBe(PageReadStatus.Success);
            messages.Length.ShouldBe(0);
        ...
        {
            var streamMessagesPage = store.ReadStreamForwards("stream-does-not-exist", StreamVersion.Start, 1);

            await using var enumerator = streamMessagesPage.GetAsyncEnumerator();
        ...
        {
            var streamMessagesPage = store.ReadStreamBackwards("stream-does-not-exist", StreamVersion.End, 1);
        ...
        public async Task When_read_deleted_stream_forwards_then_should_get_StreamNotFound()
        {
            ...
            var streamMessagesPage = store.ReadStreamForwards("stream-1", StreamVersion.Start, 1);
        ...
        public async Task When_read_deleted_stream_backwards_then_should_get_StreamNotFound()
        {
            ...
            var streamMessagesPage = store.ReadStreamBackwards("stream-1", StreamVersion.Start, 1);
        ...
        public async Task Can_read_stream_backwards_starting_past_end_of_stream(int fromVersionInclusive)
        {
            ...
            var streamMessagesPage = store.ReadStreamBackwards("stream-1", fromVersionInclusive, 1);
        ...
        public async Task when_read_stream_forwards_with_url_encodable_stream(string streamId)
        {
            ...
            var result = store.ReadStreamForwards(streamId, StreamVersion.Start, 1);

            var messages = await result.ToArrayAsync();

            Assert.Equal(streamId, messages[0].StreamId);
        }
        ...
        public async Task when_read_stream_backwards_with_url_encodable_stream(string streamId)
        {
            ...
            var result = store.ReadStreamBackwards(streamId, StreamVersion.End, 1);
