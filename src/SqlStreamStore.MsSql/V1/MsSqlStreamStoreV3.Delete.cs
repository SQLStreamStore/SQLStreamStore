namespace SqlStreamStore.V1
{
    ...
    public partial class MsSqlStreamStoreV3
    {
            ...
            CancellationToken cancellationToken)
        {
            using (var connection = _createConnection())
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    ...
                    {
                        var eventDeletedEvent = Deleted.CreateMessageDeletedMessage(sqlStreamId.IdOriginal, eventId);
            ...
            using (var connection = _createConnection())
            {
                ...
                using(var transaction = connection.BeginTransaction())
                {
                    using(var command = new SqlCommand(_scripts.DeleteStreamExpectedVersion, connection, transaction))
                    {
                        ...
                        {
                            var streamDeletedEvent = Deleted.CreateStreamDeletedMessage(streamIdInfo.SqlStreamId.IdOriginal);
                            await AppendToStreamExpectedVersionAny(
                                connection,
                                transaction,
                                SqlStreamId.Deleted,
           ...
           CancellationToken cancellationToken)
        {
            ...
            {
                var streamDeletedEvent = Deleted.CreateStreamDeletedMessage(sqlStreamId.IdOriginal);
                await AppendToStreamExpectedVersionAny(
                    connection,
                    transaction,
                    SqlStreamId.Deleted,
