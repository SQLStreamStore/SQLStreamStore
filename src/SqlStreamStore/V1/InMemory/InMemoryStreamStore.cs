    ...
    public sealed class InMemoryStreamStore : StreamStoreBase
    {
        ...
        protected override Task DeleteEventInternal(string streamId, Guid eventId, CancellationToken cancellationToken)
        {
            ...
            using (_lock.UseWriteLock())
            {
                ...
                if (deleted)
                {
                    var eventDeletedEvent = Deleted.CreateMessageDeletedMessage(streamId, eventId);
                    AppendToStreamInternal(Deleted.DeletedStreamId, ExpectedVersion.Any, new[] { eventDeletedEvent });
                }
        ...
        private void DeleteStream(string streamId, int expectedVersion)
        {
            ...
            AppendToStreamInternal(Deleted.DeletedStreamId, ExpectedVersion.Any, new[] { streamDeletedEvent });
        }
