    ...
    partial class PostgresStreamStore
    {
            ...
            CancellationToken cancellationToken) 
        {
            for (int i = 0; i < messages.Count - 1; i++)
            {
                ...
                if (expectedNextPosition != messages[i + 1].message.Position)
                {
                    Logger.InfoFormat("Gap detected.  Returning partial page {fromPosition}-{toPosition}", fromPositionInclusive, fromPositionInclusive + i + 1);
